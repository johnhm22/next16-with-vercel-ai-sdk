						...
						switch (part.type) {
							...
							case "tool-getLocation":
								switch (part.state) {
									case "input-streaming":
										return (
											<div
												key={`${message.id}-getLocation-${idx}`}
												className="bg-zinc-800/50 border border-zinc-700 rounded text-zinc-500"
											>
												<div className="text-sm text-zinc-500">
													🌤️ Receiving location request...
												</div>
									...
									case "input-available":
										return (
											<div
												key={`${message.id}-getLocation-${idx}`}
												className="bg-zinc-400/50 border border-zinc-700 rounded text-zinc-500 mb-2 mt-1 p-2"
											>
							...
							case "tool-getWeather":
								switch (part.state) {
									case "input-streaming":
										return (
											<div
												...
												className="bg-zinc-400/50 border border-zinc-700 rounded text-zinc-500 m-2 bmt-1 p-2"
											>
												<div className="text-sm text-zinc-500">
									...
									//This case is not available in the model being used
									case "input-available":
										return (
									...
									case "output-available":
										return (
											<div key={`${message.id}-getWeather-${idx}`}>
												<div className="bg-zinc-400/50 border border-zinc-700 rounded mt-1 p-2">
													<div className="text-sm text-zinc-400">
														🌤️ Weather
													</div>
													<div className="text-sm text-zinc-700">
									...
									case "output-error":
										return (
											<div key={`${message.id}-getWeather-${idx}`}>
												<div className="bg-zinc-400/50 border border-zinc-700 rounded text-zinc-500 mt-1 p-2 mb-12">
													<div className="text-sm text-red-400">
														Error: {part.errorText}
													</div>
												</div>
