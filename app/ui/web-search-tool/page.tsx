...
export default function WebSearchToolPage() {
	...
	console.log("status from ai-sdk: ", status);
	return (
				...
				<div className="mb-4" key={message.id}>
					...
					{message.parts.map((part, idx) => {
						switch (part.type) {
							...
							// case "tool-web_search_preview": //for use with openai
							case "tool-web_search": //for use with anthropic
								switch (part.state) {
			...
			>
				<div className="flex gap-2">
					...
					{status === "streaming" || status === "submitted" ? (
						<button
							onClick={stop}
							className=" disabled:opacity-50 disabled:cursor-not-allowed bg-red-500 text-white py-2 px-4 hover:bg-red-700 rounded"
							type="submit"
						>
							Stop
						</button>
